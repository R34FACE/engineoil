  ...
  try {
    ...
    const worker = await getOcrWorker((message) => {
      if (message.status === "recognizing text") {
        const innerProgress = Number.isFinite(message.progress) ? message.progress : 0;
        const totalProgress = totalPages
          ? ((completedPages + innerProgress) / totalPages) * 100
          : innerProgress * 100;
        showProgress(
          "文字を読み取っています",
          totalProgress,
          `${Math.min(completedPages + 1, totalPages)} / ${totalPages}ページ`
        );
      }
    });
    ...
    for (const file of state.files) {
      const result = isPdf(file)
        ? await processPdf(file, worker, completedPages, totalPages)
        : await processImage(file, worker, completedPages, totalPages);
    ...
    if (newRows.length === 0) {
      showMessage(
        "PDFは読めましたが、商品行を自動抽出できませんでした。OCR原文タブで文字化けや列ズレを確認してください。",
        true
      );
      activateTab("ocr");
    } else {
      showMessage(`${newRows.length}件の商品行を追加しました。粘度未判定の商品は明細で修正してください。`);
    }
  } catch (error) {
    console.error(error);
    showMessage(`読み取り中にエラーが発生しました：${getFriendlyErrorMessage(error)}`, true);
  } finally {
...
async function processPdf(file, worker, completedPages, totalPages) {
  ...
  for (let pageNumber = 1; pageNumber <= pageCount; pageNumber += 1) {
    showProgress(
      "ページを画像化しています",
      ((completedPages + pageNumber - 1) / totalPages) * 100,
      `${file.name}：${pageNumber} / ${pageCount}ページ`
    );

    const page = await pdf.getPage(pageNumber);
    const textRows = await tryExtractEmbeddedText(page, file.name, pageNumber);
    let pageRows = textRows;
    let rawText = textRows.map(formatRowAsText).join("\n");

    if (textRows.length === 0) {
      const canvas = await renderPageToCanvas(page);
      const processedCanvas = preprocessCanvas(canvas);
      const result = await worker.recognize(processedCanvas, {}, { text: true, tsv: true });
      rawText = result.data.text || "";
      const tsv = result.data.tsv || "";
      const positionalRows = parseTsvRows(
        tsv,
      ...
      const fallbackRows = parseTextRows(rawText, file.name, pageNumber);
      pageRows = positionalRows.length >= Math.max(2, fallbackRows.length * 0.55)
        ? positionalRows
        : fallbackRows;
    }

    rawParts.push(`===== ${file.name} / ${pageNumber}ページ =====\n${rawText}`);
    rows.push(...pageRows);
    page.cleanup();
  }

  await pdf.destroy();
  return { rows, rawText: rawParts.join("\n\n"), pageCount };
}
...
async function processImage(file, worker, completedPages, totalPages) {
  ...
  const tsv = result.data.tsv || "";
  const positionalRows = parseTsvRows(tsv, processedCanvas.width, processedCanvas.height, file.name, 1);
  const fallbackRows = parseTextRows(text, file.name, 1);

  return {
    rows: positionalRows.length >= Math.max(2, fallbackRows.length * 0.55)
      ? positionalRows
      : fallbackRows,
    rawText: `===== ${file.name} / 1ページ =====\n${text}`,
    pageCount: 1,
  };
}

async function tryExtractEmbeddedText(page, sourceFile, pageNumber) {
  try {
    ...
    return lines.flatMap((line) => {
      ...
      const parsed = parseTextLine(text, sourceFile, pageNumber);
...
async function getOcrWorker(logger) {
  ...
  if (!window.Tesseract) {
    throw new Error("Tesseract.jsを読み込めませんでした。通信環境を確認してから再読み込みしてください。");
  }

  const oem = window.Tesseract.OEM?.LSTM_ONLY ?? 1;
  const psm = window.Tesseract.PSM?.SINGLE_BLOCK ?? "6";

  // Tesseract.js v5以降は createWorker の言語指定が文字列です。
  // 配列を渡すと、ブラウザによってOCR開始前にエラーになります。
...
function preprocessCanvas(sourceCanvas) {
  ...
  for (let i = 0; i < data.length; i += 4) {
    ...
    const value = gray > 242 ? 255 : Math.max(0, Math.min(255, (gray - 128) * 1.35 + 128));
    data[i] = value;
    data[i + 1] = value;
    data[i + 2] = value;
  }
...
function parseTsvRows(tsv, width, height, sourceFile, pageNumber) {
  ...
  const nameEnd = width * (clamp(Number(els.productNameEndInput.value), 65, 85, 78.5) / 100);
  const rows = [];
  ...
  for (const cluster of clusters) {
    ...
    const codeIndex = lineWords.findIndex((word) => {
      ...
      return code.length === 8 && ratio >= 0.25 && ratio <= 0.62;
    });

    if (codeIndex < 0) continue;

    const codeWord = lineWords[codeIndex];
    const code = normalizeProductCode(codeWord.text);
    const quantityWord = lineWords
      .filter((word) => Math.abs(word.centerX - quantityX) <= quantityTolerance && isIntegerToken(word.text))
      .sort((a, b) => Math.abs(a.centerX - quantityX) - Math.abs(b.centerX - quantityX))[0];

    if (!quantityWord) continue;

    const quantity = parseInteger(quantityWord.text);
    if (!Number.isFinite(quantity) || quantity < 0 || quantity > 100000) continue;

    const productName = cleanProductName(
      lineWords
        .filter((word) => word.left > codeWord.left + codeWord.width * 0.6 && word.centerX < nameEnd)
        .map((word) => word.text)
        .join(" ")
    );
    ...
    rows.push(makeRow({ sourceFile, pageNumber, productCode: code, productName, quantity }));
  }
...
function parseTextRows(text, sourceFile, pageNumber) {
  ...
  for (let index = 0; index < lines.length; index += 1) {
    ...
    for (const candidate of candidates) {
      const parsed = parseTextLine(candidate, sourceFile, pageNumber);
      if (parsed) {
        rows.push(parsed);
        break;
      }
...
function parseTextLine(line, sourceFile, pageNumber) {
  ...
  const match = rest.match(/^(.+?\S)\s+([\d,]+)\s+([\d,]+)\s+([\d,]+)$/);
  if (!match) return null;

  const productName = cleanProductName(match[1]);
  const quantity = parseInteger(match[3]);

  if (!productName || !Number.isFinite(quantity) || quantity < 0 || quantity > 100000) return null;

  return makeRow({ sourceFile, pageNumber, productCode, productName, quantity });
}

function formatRowAsText(row) {
  return `${row.productCode} ${row.productName} ${row.quantity}`;
}
...
function normalizeProductCode(value) {
  const digits = String(value || "")
    ...
    .replace(/[IL|]/g, "1")
    .replace(/\D/g, "");
  return digits.length >= 7 && digits.length <= 8 ? digits.padStart(8, "0") : digits.slice(0, 8);
}
...
function detectViscosity(productName) {
  ...
  const match = text.match(/(?:^|[^0-9])((?:0|5|10|15|20))\s*W\s*-?\s*(8|12|16|20|30|40|50)(?:[^0-9]|$)/);
  return match ? `${Number(match[1])}W-${Number(match[2])}` : "粘度不明";
}

function detectCategory(productName) {
  const text = String(productName || "").normalize("NFKC").toUpperCase();
  return /(量り売り|量売り|量売|L迄|Lまで|レデマ|ﾚﾃﾞﾏ|L迄|Lまで)/i.test(text) ? "量り売り" : "缶商品";
}
...
function dedupePageRows(rows) {
  const map = new Map();
  rows.forEach((row) => {
    const key = [row.sourceFile, row.pageNumber, row.productCode, normalizeCompareText(row.productName), row.quantity].join("|");
...
function renderAll() {
  if (state.rows.length > 0) els.resultsSection.classList.remove("hidden");
  renderKpis();
...
function getVisibleRows() {
  ...
  return state.rows.filter((row) => {
    ...
    return [row.viscosity, row.productName, row.productCode, row.sourceFile].some((value) =>
      normalizeCompareText(value).includes(search)
    );
  });
}
...
function renderKpis() {
  ...
  els.totalQuantityKpi.textContent = total.toLocaleString("ja-JP");
  ...
  els.unknownCountKpi.textContent = unknown.toLocaleString("ja-JP");
}

function renderViscosityFilter() {
  const current = els.viscosityFilter.value;
  const values = [...new Set(state.rows.map((row) => row.viscosity))].sort(compareViscosity);
...
function groupSummary(rows) {
  ...
  rows.forEach((row) => {
    ...
    const key = row.productCode && row.productCode !== "00000000"
      ? row.productCode
      : normalizeCompareText(row.productName);

    if (!group.productMap.has(key)) {
      group.productMap.set(key, {
    ...
    const product = group.productMap.get(key);
  ...
  groups.forEach((group, index) => {
    ...
    details.innerHTML = `
      <summary>
        ...
        <span class="viscosity-total"><strong>${group.totalQuantity.toLocaleString("ja-JP")}</strong><span>販売数量合計</span></span>
      </summary>
...
function reparseRawText() {
  ...
  const rows = [];
  chunks.forEach((chunk, index) => rows.push(...parseTextRows(chunk, "OCR原文再解析", index + 1)));
...
function activateTab(name) {
  ...
  ({ summary: $("#summaryTab"), detail: $("#detailTab"), ocr: $("#ocrTab") })[name]?.classList.add("active");
}
...
function exportSummaryCsv() {
  ...
  groups.forEach((group) => {
    group.products.sort((a, b) => b.quantity - a.quantity).forEach((product) => {
...
function exportJson() {
  downloadBlob(
    JSON.stringify({ version: 2, exportedAt: new Date().toISOString(), rows: state.rows, rawText: state.rawText }, null, 2),
...
function getFriendlyErrorMessage(error) {
  ...
  if (/worker|createWorker|language|traineddata/i.test(message)) {
    return `${message}。OCRデータの読み込みに失敗した可能性があります。ページを再読み込みして再試行してください。`;
  }
  if (/password/i.test(message)) return "パスワード付きPDFは読み込めません。";
  if (/Invalid PDF|Missing PDF|PDF/i.test(message)) return `${message}。PDFが破損しているか、ブラウザで処理できない形式です。`;
  return message;
}
